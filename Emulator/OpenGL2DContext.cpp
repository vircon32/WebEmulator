...
void OpenGL2DContext::CreateOpenGLWindow()
{
    ...
    SDL_GL_LoadLibrary( nullptr );
      ...
      // WebGL 1 / GLES 2.0 — required by LEGACY_GL_EMULATION fixed-pipeline path
      SDL_GL_SetAttribute( SDL_GL_CONTEXT_MAJOR_VERSION, 2 );
      SDL_GL_SetAttribute( SDL_GL_CONTEXT_MINOR_VERSION, 0 );
      // Double buffering is required for WebGL presentation
    ...
    // Set up an orthographic projection matrix so that glOrtho works
    // correctly with LEGACY_GL_EMULATION. Without explicitly selecting
    // the projection matrix first, the FFP emulation layer may apply
    // the ortho transform to MODELVIEW instead, which leaves the
    // projection matrix as identity and causes nothing to render.
    ...
    // switch back to modelview for all subsequent draw calls
...
void OpenGL2DContext::SetFullScreen()
{
    ...
    #ifdef __EMSCRIPTEN__
        // Emscripten doesn't support fullscreen in the same way
    ...
    // update the projection matrix to match the new viewport size
    glMatrixMode( GL_PROJECTION );
    glLoadIdentity();
    glOrtho( 0, WindowWidth, WindowHeight, 0, -1, 1 );
    glMatrixMode( GL_MODELVIEW );
...
void OpenGL2DContext::ExitFullScreen()
{
    ...
    // update the projection matrix to match the restored viewport
    glMatrixMode( GL_PROJECTION );
    glLoadIdentity();
    glOrtho( 0, WindowWidth, WindowHeight, 0, -1, 1 );
    glMatrixMode( GL_MODELVIEW );
...
{
    // Note: in Emscripten LEGACY_GL_EMULATION, glColor4ub routes internally to
    // glVertexAttrib4f at index (GL_MAX_TEXTURE_IMAGE_UNITS * 2).
    // The compile flag -s GL_MAX_TEXTURE_IMAGE_UNITS=1 keeps that index at 2,
    // within WebGL 1's MAX_VERTEX_ATTRIBS limit (0-15).
    glColor4ub
    (
        MultiplyColor.R,
        MultiplyColor.G,
