// *****************************************************************************
    ...
    #include <cstring>          // [ ANSI C ] Memory operations (memcpy)
...
void VirconEmulator::LoadCartridge( const std::string& FilePath )
{
    ...
    // store cartridge media counts in controller so the CPU can query them
...
// =============================================================================
//      VIRCON EMULATOR: WEB MEMORY LOADING
// =============================================================================

void VirconEmulator::LoadCartridgeFromMemory( const unsigned char* Data, int Size )
{
    cout << "Loading cartridge from memory (" << Size << " bytes)" << endl;
    ...
    // ensure that we can at least load the file header
    if( Size < (int)sizeof(ROMFileHeader) )
      throw runtime_error( "Incorrect V32 file format (file is too small)" );
    
    // read the global header
    ROMFileHeader ROMHeader;
    std::memcpy( &ROMHeader, Data, sizeof(ROMFileHeader) );
    ...
    if( (uint32_t)Size != SizeAfterAudioROM )
      throw runtime_error( "Incorrect V32 file format (file size does not match indicated ROM contents)" );
    ...
    // load a binary file signature
    BinaryFileHeader BinaryHeader;
    std::memcpy( &BinaryHeader, Data + sizeof(ROMFileHeader), sizeof(BinaryFileHeader) );
    ...
    // check program rom size limitations
    if( !IsBetween( BinaryHeader.NumberOfWords, 1, Constants::MaximumCartridgeProgramROM ) )
      throw runtime_error( "Cartridge program ROM size is out of valid range" );
    
    // load the binary contents into a temporary buffer
    vector< VirconWord > LoadedBinary;
    LoadedBinary.resize( BinaryHeader.NumberOfWords );
    std::memcpy( &LoadedBinary[0], 
                  Data + sizeof(ROMFileHeader) + sizeof(BinaryFileHeader),
                  BinaryHeader.NumberOfWords * sizeof(VirconWord) );
    
    // Connect the cartridge with the binary data
    CartridgeController.Connect( &LoadedBinary[0], BinaryHeader.NumberOfWords );
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // STEP 4: Load video ROM  (use a sequential cursor — each texture has variable size)
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    
    // Start cursor at the video ROM section
    uint32_t VideoCursor = ROMHeader.VideoROMLocation.StartOffset;
    
    for( unsigned i = 0; i < ROMHeader.NumberOfTextures; i++ )
    {
        TextureFileHeader TextureHeader;
        std::memcpy( &TextureHeader, Data + VideoCursor, sizeof(TextureFileHeader) );
        VideoCursor += sizeof(TextureFileHeader);
        ...
        if( !IsBetween( TextureHeader.TextureWidth , 0, 1024 )
        ||  !IsBetween( TextureHeader.TextureHeight, 0, 1024 ) )
          throw runtime_error( "Cartridge texture dimensions are out of range" );
        
        uint32_t TexturePixels = TextureHeader.TextureWidth * TextureHeader.TextureHeight;
        
        vector< VirconWord > LoadedTexture( TexturePixels );
        std::memcpy( &LoadedTexture[0], Data + VideoCursor, TexturePixels * sizeof(VirconWord) );
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // STEP 5: Load audio ROM  (sequential cursor)
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    
    uint32_t AudioCursor = ROMHeader.AudioROMLocation.StartOffset;
    uint32_t TotalSPUSamples = 0;
    
    for( unsigned i = 0; i < ROMHeader.NumberOfSounds; i++ )
    {
        SoundFileHeader SoundHeader;
        std::memcpy( &SoundHeader, Data + AudioCursor, sizeof(SoundFileHeader) );
        AudioCursor += sizeof(SoundFileHeader);
        
        if( !CheckSignature( SoundHeader.Signature, Signatures::SoundFile ) )
          throw runtime_error( "Cartridge sound does not have a valid signature" );
        
        if( !IsBetween( SoundHeader.SoundSamples, 1, Constants::SPUMaximumCartridgeSamples ) )
          throw runtime_error( "Cartridge sound length is out of range" );
        
        TotalSPUSamples += SoundHeader.SoundSamples;
        if( TotalSPUSamples > (uint32_t)Constants::SPUMaximumCartridgeSamples )
          throw runtime_error( "Cartridge total sound samples exceed SPU limit" );
        
        vector< SPUSample > LoadedSound( SoundHeader.SoundSamples );
        std::memcpy( &LoadedSound[0], Data + AudioCursor, SoundHeader.SoundSamples * sizeof(SPUSample) );
        AudioCursor += SoundHeader.SoundSamples * sizeof(SPUSample);
    ...
    cout << "Cartridge loaded successfully from memory" << endl;
}

// -----------------------------------------------------------------------------

void VirconEmulator::LoadMemoryCardFromMemory( const unsigned char* Data, int Size )
{
    cout << "Loading memory card from memory (" << Size << " bytes)" << endl;
    
    // unload any previous memory card
    UnloadMemoryCard();
    
    // Validate size: 8-byte signature + MemoryCardSize 32-bit words
    int ExpectedBytes = 8 + Constants::MemoryCardSize * 4;
    if( Size != ExpectedBytes )
      throw runtime_error( "Memory card file does not match the expected Vircon memory card size" );
    
    // Check signature (first 8 bytes)
    if( !CheckSignature( (char*)Data, Signatures::MemoryCardFile ) )
      throw runtime_error( "Memory card file does not have a valid signature" );
    
    // Connect memory and copy data (skip 8-byte signature header)
    MemoryCardController.Connect( Constants::MemoryCardSize );
    std::memcpy( &MemoryCardController.Memory[0], Data + 8, Constants::MemoryCardSize * 4 );
    
    // Set a MEMFS path so SaveContents / flushMemoryCard work correctly
    MemoryCardController.CardSavePath = "/uploads/memorycard.v32";
    
    cout << "Memory card loaded successfully from memory" << endl;
}
