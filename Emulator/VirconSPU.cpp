...
void VirconSPU::LaunchPlaybackThread()
{
    ...
    // Emscripten does not support SDL threads without SharedArrayBuffer/pthreads.
    // Audio buffer management is driven from the main loop (ChangeFrame) instead.
    // Leave PlaybackThread null — the thread functions QueueFilledBuffers /
    // UnqueuePlayedBuffers will be called directly from VirconSPU::ChangeFrame.
    PlaybackThread = nullptr;
    #else
...
void VirconSPU::ChangeFrame()
{
    ...
    #ifdef __EMSCRIPTEN__
    // Without a background playback thread, we drive buffer management from
    // the main loop directly. Both functions are thread-safe as long as they
    // are only called from one thread at a time (which is the case here).
    QueueFilledBuffers();
